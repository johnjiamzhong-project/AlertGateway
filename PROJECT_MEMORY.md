...
## 2026-07-21 主分支与ROI分支边界

- `master` 保留本次多路NPU调度、4K/1080p处理、通用多路管线、公共测试工具和性能文档；主分支不再以ROI实验入口作为默认启动方式。
- `feature_roi` 保留此前的ROI调试提交及其专用实验入口。ROI通用跟踪/框过滤能力作为现有检测管线的公共能力保留，但ROI候选配置 `config/config_4k_roi_candidate_20260720.json` 和ROI默认 `run.sh` 不进入本次主分支提交。
- 因当前多路调度代码依赖 `ChannelPipeline`、`FrameBufferPool` 和通道化 `ModelConfig` 等公共基础，本次主分支提交会包含这些基础实现，但不合并ROI分支的独立提交历史。
