...
struct ModelConfig {
    ...
    std::string output_layout = "decoded";    // decoded=当前2输出；rockchip_dfl=官方9输出
};

struct DetectionConfig {
    int report_interval_sec = 1;  // MQTT上报的最小间隔；配合 last_summary_key_ 做"变化时才发"去重
};
...
private:
    ...
    std::string summary_key(const std::vector<Detection>& dets);  // 不含timestamp，用于MQTT去重
    ...
    int                              box_output_idx_ = -1;  // 双输出模型的box/cls index
    ...
    std::string                             last_summary_key_;  // 上次推送MQTT的内容签名，用于去重
