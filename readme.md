...
经过多轮性能调优，目前单帧端到端推理总耗时约 35ms（NPU 推理本身 ~30ms，已对标官方 demo 验证为该 NPU 硬件极限），编码推流稳定运行在摄像头实际帧率，RTMP 端到端延迟 < 1s，支持断线自动重连。
...
- 单帧 NPU 推理稳定在 ~30ms，已用瑞芯微官方 demo 在同型号 NPU、同模型上交叉验证，确认是当前硬件可达到的性能上限
