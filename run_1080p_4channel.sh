#!/usr/bin/env bash
# Four 1080p publishers -> manually started SRS -> decode to 1080p -> one global-Scheduler process.
...
VIDEO_A="${VIDEO_A:-runs/input_videos/1080p/VID_20260712_131214.mp4}"
VIDEO_B="${VIDEO_B:-runs/input_videos/1080p/VID_20260712_131410.mp4}"
...
CONFIG="${CONFIG:-config/config_multi_4ch_scheduler_1080p_candidate.json}"
RUN_ID="$(date +%Y%m%d_%H%M%S)"
LOG_DIR="${LOG_DIR:-/tmp/alertgateway_1080p_4channel_${RUN_ID}}"
REMOTE_BINARY="/tmp/AlertGateway.1080p_4channel_${RUN_ID}"
REMOTE_CONFIG="/tmp/alertgateway_1080p_4channel_${RUN_ID}.json"
REMOTE_LOG="/tmp/alertgateway_1080p_4channel_${RUN_ID}.log"

INPUT_URLS=(
  "rtmp://192.168.0.168/live/alertgateway_1080p_source_a"
  "rtmp://192.168.0.168/live/alertgateway_1080p_source_b"
  "rtmp://192.168.0.168/live/alertgateway_1080p_source_c"
  "rtmp://192.168.0.168/live/alertgateway_1080p_source_d"
)
RESULT_URLS=(
...
STREAM_NAMES=(alertgateway_1080p_source_a alertgateway_1080p_source_b alertgateway_1080p_source_c alertgateway_1080p_source_d)
...
expected_width = int(sys.argv[3]) if sys.argv[3] else None
expected_height = int(sys.argv[4]) if sys.argv[4] else None
for item in streams:
    if item.get("name") != sys.argv[2]:
        continue
    if not (item.get("active") or item.get("publish", {}).get("active")):
        continue
...
wait_stream() {
  local name="$1" timeout_sec="$2" output="$3" expected_width="${4:-}" expected_height="${5:-}"
  local deadline=$((SECONDS + timeout_sec))
  while (( SECONDS < deadline )); do
    if fetch_srs "$output" && stream_active "$output" "$name" "$expected_width" "$expected_height"; then return 0; fi
    sleep 2
  done
  return 1
}
...
log "=== 四路 1080p 输入 -> 1080p 原始解码 NPU Scheduler 测试 ==="
log "配置：$CONFIG；运行 ${RUN_SEC} 秒；ROI/Tiling/Thumbnail 全部关闭"
log "日志目录：$LOG_DIR"
...
for index in 0 1 2 3; do
  wait_stream "${STREAM_NAMES[$index]}" 35 "$LOG_DIR/srs_input_$((index + 1)).json" 1920 1080 \
    || fail "第 $((index + 1)) 路 1080p 输入未 active"
done

log "四路 1080p 输入均已 active，启动单进程全局 NPU Scheduler（处理分辨率 1920x1080）。"
ssh -o BatchMode=yes -o ConnectTimeout=5 "$BOARD_HOST" \
...
for channel in 1 2 3 4; do
  wait_stream "alertgateway_channel_${channel}" 45 "$LOG_DIR/srs_result_${channel}.json" \
    || fail "第 ${channel} 路 1080p 结果流未 active"
done
...
wait "$BOARD_SSH_PID" || fail "板端四路 1080p Scheduler 异常退出"
BOARD_STARTED=0
stop_srs_watch
stop_publishers
